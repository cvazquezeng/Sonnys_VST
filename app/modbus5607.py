...
def turn_off_lights5607(start_address, end_address):
    for address in range(start_address, end_address + 1):
        control_lights5607(address, False)
        logging.debug(f"Turning off address: {address}")

def read_coils_status5607(offset_value, selected):
    try:
